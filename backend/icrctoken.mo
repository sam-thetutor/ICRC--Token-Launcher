...
actor class Ledger(init : { initial_mints : [{ account : { owner : Principal; subaccount : ?Blob }; amount : Nat }]; minting_account : { owner : Principal; subaccount : ?Blob }; token_name : Text; token_symbol : Text; decimals : Nat8;token_logo:Text; transfer_fee : Nat }) = this {
...
//   public func getAllTokenHolders():async [(Account,Nat)]{

//     var tempData = Buffer.Buffer<(Account,Nat)>(0);

//     for (data in log.vals()){

//       let tokBal = balance(data.)


//     };


// return Buffer.toArray<(Account,Nat)>(tempData);
//   };


  // //function to return all the token transaction history

  public query func getTransactionHistory():async [Transaction]{
    return Buffer.toArray<Transaction>(log)
  };
  ...
  public query func icrc1_logo() : async Text {
    init.token_logo;
  };
  ...
  public query func icrc1_metadata() : async [(Text, Value)] {
    [
      ...
      ("icrc1:logo", #Text(init.token_logo)),
